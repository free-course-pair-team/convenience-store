...
import store.domain.entity.Receipt
...
class OutputView {
    ...
    fun ReceiptProductInfo(purchaseProductInfos: List<PurchaseCompleteProduct>) {
        val promotionTotalCount = purchaseProductInfos.map { it.promotionProduct.promotionCount + it.promotionProduct.noPromotionCount + it.promotionProduct.freeCount }
        println("===========W 편의점=============")
        println("상품명\t\t수량\t금액")
        purchaseProductInfos.forEachIndexed() { index, purchaseProductInfo ->
            ...
            println("${purchaseProductInfo.name} ${productCount} ${productCount * purchaseProductInfo.price}")
        }
    }

    fun ReceiptPromotionInfo(purchaseProductInfos: List<PurchaseCompleteProduct>) {
    ...
    fun ReceiptTotalInfo(purchaseProductInfos: List<PurchaseCompleteProduct>, membershipResult: Int) {
        val promotionTotalCount = purchaseProductInfos.map { it.promotionProduct.promotionCount + it.promotionProduct.noPromotionCount + it.promotionProduct.freeCount }
        ...
        val promotionDiscount = purchaseProductInfos.map { it.promotionProduct.freeCount * it.price }.sum()
